...
class TICED(nn.Module):
    ...
    def forward(self, seq, lengths):
        ...
        ini_atten = self.tanh(ini_atten)  
        ini_w = self.v1(ini_atten).expand_as(ini_emb) * ini_emb 

        
        layer1mask = mask.unsqueeze(2).expand_as(ini_emb) * ini_w   
        layer1mask = torch.sum(layer1mask, 1)
        ...
        beta1_mask = beta1 - self.lambdas * beta1_v   
        ...
        layer2cur = self.final2std2_cur(embs_final.contiguous().view(-1, self.embedding_dim)).view(embs_final.size()) 
        layer2_a = self.final2std2_std(sess_std) 
        layer2_a_expand = layer2_a.unsqueeze(1).expand_as(layer2cur) 
        layer2mask = mask.unsqueeze(2).expand_as(layer2cur) * layer2_a_expand 
